...
if [ -d /var/lib/mysql/mysql ]; then
	...
	mysql_install_db --user=mysql --ldata=/var/lib/mysql --basedir=/usr --datadir=/opt/tmpfs > /dev/null
...
EOF
	...
	echo "exec /usr/bin/mysqld --user=mysql --console --skip-name-resolve --skip-networking=0 --datadir=$TMPFS_DIR" "$@"
fi
...
exec /usr/bin/mysqld --user=mysql --console --skip-name-resolve --skip-networking=0 --datadir=$TMPFS_DIR $@